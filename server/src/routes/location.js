...
export default async function locationRoutes(fastify) {
  ...
  // Get partner's last known position
  fastify.get('/share/:recordId', { preHandler: authenticate }, async (req, reply) => {
    ...
    const partnerId = record.requesterId === req.userId ? record.consenterId : record.requesterId;
    const share = await prisma.locationShare.findUnique({
      where: { recordId_userId: { recordId: req.params.recordId, userId: partnerId } },
    });
